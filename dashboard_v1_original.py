...
with tab2:
    ...
    with col2:
        st.markdown('<div class="section-header">??????? Market Share (Treemap)</div>', unsafe_allow_html=True)
        fig_tree = px.treemap(
...
with tab4:
    ...
    with col2:
        ...
        st.markdown('<div class="section-header">??????? Revenue per Store by Country</div>', unsafe_allow_html=True)
...
with tab5:
    ...
    # ?????? Season factor
    st.markdown('<div class="section-header">??????? Season Factor Distribution</div>', unsafe_allow_html=True)
    fig_sf = px.histogram(
